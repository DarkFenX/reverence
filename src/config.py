...
class Config(object):
    ...
    __tables__ = (
        ...
        ("ramcompletedstatuses",
         (276, 0, IndexRowset, RamCompletedStatus, "completedStatus", const.cacheRamCompletedStatuses)),
        ("ramaltypesdetailpercategory",
         (0, 0, FilterRowset, RamDetail, "assemblyLineTypeID", const.cacheRamAssemblyLineTypesCategory)),
        ("ramaltypesdetailpergroup",
         (0, 0, FilterRowset, RamDetail, "assemblyLineTypeID", const.cacheRamAssemblyLineTypesGroup)),
        ...
        ("mapcelestialdescriptions",
         (276, 0, IndexRowset, MapCelestialDescription, "itemID", const.cacheMapCelestialDescriptions)),
        ...
        ("mapFactionsOwningSolarSystems",
         (393, 0, ("factionsOwningSolarSystems", "factionsOwningSolarSystems", False), None)),
        ...
        ("invcategories", (412, 0, ("evecategories.static", InvCategory))),
        ("invgroups", (412, 0, ("evegroups.static", InvGroup))),
        ("invtypes", (412, 0, ("evetypes.static", InvType))),
    )
    ...
    def eveowners(self):
        ...
        rs = IndexRowset(['ownerID', 'ownerName', 'typeID', 'gender', 'ownerNameID'], None, key="ownerID",
                         RowClass=EveOwners, cfgInstance=self)
        d = rs.items
        ...
        for row in self.cache.LoadBulk(const.cacheChrNpcCharacters):
            ...
            d[id_] = DBRow(rd, [id_, row.characterName, bloodlinesToTypes[row.bloodlineID], row.gender,
                                row.characterNameID])
    ...
    def evelocations(self):
        rs = IndexRowset(['locationID', 'locationName', 'x', 'y', 'z', 'locationNameID'], None, key="locationID",
                         RowClass=EveLocations, cfgInstance=self)
    ...
    def __init__(self, eve):
        ...
        self._tables = dict(
            ((k, v) for k, v in self.__tables__ if protocol >= v[0] and protocol < (v[1] or 2147483647)))
...
which will be called as func(current, total, tableName).
    ...
    def GetCelestialNameFromLocalRow(self, row):
        ...
        if celestialNameID is not None and celestialGroupID != const.groupStargate:
            ...
            param = {'solarSystemID': row['solarSystemID'],
                     'romanCelestialIndex': util.IntToRoman(row['celestialIndex']), 'typeID': row['typeID'],
                     'orbitIndex': row['orbitIndex']}
        elif celestialGroupID == const.groupMoon:
            label = 'UI/Locations/LocationMoonFormatter'
            param = {'solarSystemID': row['solarSystemID'],
                     'romanCelestialIndex': util.IntToRoman(row['celestialIndex']), 'orbitIndex': row['orbitIndex']}
    ...
    def GetNPCStationNameFromLocalRow(self, row):
        operationName = self._localization.GetByMessageID(
            self.staoperationtypes.Get(row['operationID']).operationNameID) if row['useOperationName'] else ""
        return self._localization.GetByLabel('UI/Locations/LocationNPCStationFormatter', orbitID=row['orbitID'],
                                             corporationID=row['ownerID'], operationName=operationName)
