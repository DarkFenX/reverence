...
_SchemaLoader.add_constructor(u'tag:yaml.org,2002:map',
                              lambda loader, node: collections.OrderedDict(
                                  map(loader.construct_object, kv) for kv in node.value))
...
class _FixedSizeList(object):
    ...
    def __iter__(self):
        d = self.data;
        s = self.itemSchema;
        loader = self.itemSchema['loader']
        return (loader(d, offset, s) for offset in
                xrange(self.offset, self.offset + self.count * self.itemSize, self.itemSize))
...
class _VariableSizedList(object):
    ...
    def __getitem__(self, idx):
        ...
        if idx < 0 or idx >= self.count:
            raise IndexError('Invalid item index %i for list of length %i' % (idx, self.count))
        return self.itemSchema['loader'](self.data, self.start + _uint32(self.data, self.offset + idx * 4),
                                         self.itemSchema)
...
def FSD_List(data, offset, schema):
    return (_FixedSizeList if 'fixedItemSize' in schema else _VariableSizedList)(data, offset, schema['itemTypes'],
                                                                                 schema.get('length'))
...
class FSD_Dict(object):
    ...
    def __repr__(self):
        return "<FSD_Dict(keys:%s,values:%s,size:%d)>" % (
        self.schema['keyTypes']['type'], self.valueSchema['type'], len(self.footer))
...
class _subindex(object):
    ...
    def __getitem__(self, key):
        ...
        if self.fsdindex:
            return FSD_Index(self.file, 100, self.valueSchema, offset=offsetAndSize[0],
                             offsetToFooter=offsetAndSize[0] + offsetAndSize[1])
    ...
    def iterkeys(self):
        return self.offsetTable.iterkeys()

    def itervalues(self):
        return (self[key] for key in self.offsetTable.iterkeys())

    def iteritems(self):
        return ((key, self[key]) for key in self.offsetTable.iterkeys())

    def __len__(self):
        return len(self.offsetTable)

    def __contains__(self):
        return key in self.offsetTable
...
class _indexgroup(object):
    ...
    def iterkeys(self):
        return _chain(self.indices)

    def itervalues(self):
        return (v for index in self.indices for v in index.itervalues())

    def iteritems(self):
        return (kv for index in self.indices for kv in index.iteritems())
...
class FSD_MultiIndex(FSD_Index):
    ...
    def __repr__(self):
        ...
        return "<FSD_MultiIndex(type:%s,size:%d,indices:{%s})>" % (
        self.valueSchema['type'], len(self.footer), ','.join(indexsummary))
...
class FSD_Object(object):
    ...
    def __repr__(self):
        ...
        return "<FSD_Object(" + ','.join(
            map(u"%s:%s".__mod__, ((k, v['type']) for k, v in self.attributes.iteritems()))) + ")>"
