...
def FmtAmt(amount, fmt="ln", showFraction=0, fillWithZero=0):
    ...
    if fmt[0] == "l":
        ...
        if amount >= 10000:
            ...
            amount = TruncateAmt(amount, 10 ** order) + (
            symbol if (fmt[1] != "l") else canonical[str(amount)[0] != "1"])
...
def IsWormholeConstellation(constellationID):
    return (constellationID >= const.mapWormholeConstellationMin) and (
    constellationID < const.mapWormholeConstellationMax)
